...
*   **ヘッダー構成:**
    *   **左側:** アプリケーションロゴ・タイトル
    *   **右側:** ログインユーザーアイコン（アバター）
    *   **ユーザーメニュー:** ユーザーアイコンクリックで以下のドロップダウンメニューを表示
        *   **⚙️ 設定:** 設定モーダル（`LAW-SC-300`）を開く
        *   **🚪 ログアウト:** ログアウト確認ダイアログ表示後、ログアウト実行してログイン画面へ遷移
...
##### 3.2.7. 設定モーダル - `LAW-SC-300`
*   **概要:** アプリケーション全体の設定を管理するモーダル。ユーザーの設定やテーマ設定などを一元管理する。
*   **ワイヤーフレーム:**
    ```
    +--------------------------------------------------------------------------+
    |  [ 設定 ]                                                       [ × ]   |
    ...
    |  [ アカウント設定 ]                                                      |
    |  +--------------------------------------------------------------------+  |
    |  | ユーザー名: [ input: 現在のユーザー名 ]                             |  |
    |  | メールアドレス: [ input: user@example.com ]                         |  |
    |  +--------------------------------------------------------------------+  |
    |                                                                          |
    |  [ その他 ]                                                              |
    |  +--------------------------------------------------------------------+  |
...
*   **機能:**
    *   **テーマ設定:**
        *   **ライト:** ライトテーマを適用
        *   **ダーク:** ダークテーマを適用  
        *   **システム設定に従う:** ユーザーのOS/ブラウザの設定に従ってテーマを自動切り替え
    *   **アカウント設定:** ユーザーの基本情報を編集
    *   **その他設定:** アプリケーション情報の表示、フィードバック機能
